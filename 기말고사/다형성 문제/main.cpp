...
//--------
{
	...
	/*
	Shape 클래스에는 static int gid가 있어 정적 멤버 변수는 클래스당 하나만 생성되므로,
	해당 변수는 한 번만 계산됨에 따라 4바이트(32비트 시스템) 또는 8바이트(64비트 시스템)임

	Shape 클래스에는 int id가 있어 이는 정적이 아닌 멤버 변수이므로 각 객체마다 따로 할당됨.
	4바이트(32비트 시스템) 또는 8바이트(64비트 시스템)임
