...
//--------------------------------------------------------------------------------------
	...
	for (int i = 0; i < num; ++i) {
		...
		switch (sel) {
		case 0:
			mon[i] = new TimeMonster(i);
			break;
		case 1:
			mon[i] = new MemoryMonster(i);
			break;
		case 2:
			mon[i] = new ScreenMonster(i);
			break;
		default:
			// 예외 처리 등을 추가할 수 있음
			break;
...
// [문제 5] num개의 Monster 중에서 MemoryMonster를 찾아 special 함수를 두 번 호출하라.	(20)
	...
	for (int i = 0; i < num; ++i) {
		MemoryMonster* memoryMonster = dynamic_cast<MemoryMonster*>(mon[i]);
		if (memoryMonster) {
			memoryMonster->special();
			memoryMonster->special();
		}
	}
